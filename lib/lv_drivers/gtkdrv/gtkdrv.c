...
void gtkdrv_init(int hor_res, int ver_res)
{
    // Init GTK
    gtk_init(NULL, NULL);
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), hor_res, ver_res);
    ...
    pixbuf = gdk_pixbuf_new_from_data((guchar*)fb, GDK_COLORSPACE_RGB, false, 8, hor_res, ver_res, hor_res * 3, NULL, NULL);
    if (pixbuf == NULL)
    {
...
//    pthread_create(&thread, NULL, gtkdrv_handler, NULL);
    gtkdrv_task = lv_task_create(gtkdrv_handler, 1, LV_TASK_PRIO_MID, NULL);
//         gtkdrv_Handler(NULL);
        gtk_main_iteration_do(FALSE);

//    lv_task_ready(gtkdrv_task);
}
...
void gtkdrv_handler(lv_task_t *task)
{
//    while(1) {
